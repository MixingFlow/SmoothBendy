...
public sealed class AuthInterface : Handle
{
	...
	{
		IntPtr source = Bindings.EOS_Auth_GetLoggedInAccountByIndex(base.InnerHandle, index);
		Helper.TryMarshalGet(source, out EpicAccountId target);
		return target;
	}
	...
	public EpicAccountId GetMergedAccountByIndex(EpicAccountId localUserId, uint index)
	{
		IntPtr target = IntPtr.Zero;
		Helper.TryMarshalSet(ref target, localUserId);
		IntPtr source = Bindings.EOS_Auth_GetMergedAccountByIndex(base.InnerHandle, target, index);
