...
public static class Common
{
	...
	{
		int source = Bindings.EOS_EResult_IsOperationComplete(result);
		Helper.TryMarshalGet(source, out var target);
		return target;
	}

	public static string ToString(Result result)
	{
		IntPtr source = Bindings.EOS_EResult_ToString(result);
		Helper.TryMarshalGet(source, out string target);
		return target;
	}
