...
public static class Helper
{
	...
	{
		return TryFetchHandle<THandle>(source, out target, (int)arrayLength);
	}
	...
	{
		IEnumerable<KeyValuePair<IntPtr, DelegateHolder>> source = s_Callbacks.Where(delegate(KeyValuePair<IntPtr, DelegateHolder> pair)
		{
			int result;
			if (pair.Value.NotificationId.HasValue)
			{
				ulong? notificationId2 = pair.Value.NotificationId;
				result = ((notificationId2.GetValueOrDefault() == notificationId && notificationId2.HasValue) ? 1 : 0);
			}
			else
			{
				result = 0;
	...
	private static bool TryFetchHandle<THandle>(IntPtr source, out THandle[] target, int arrayLength) where THandle : Handle, new()
	{
		...
		for (int i = 0; i < arrayLength; i++)
		{
			...
			ptr = Marshal.ReadIntPtr(ptr);
			TryConvert<THandle>(ptr, out var target2);
			list.Add(target2);
		}
		target = list.ToArray();
		return true;
	...
	private static bool TryConvert(DateTimeOffset? source, out long target)
	{
		...
		if (source.HasValue)
		{
			...
			long ticks = (source.Value.UtcDateTime - dateTime).Ticks;
