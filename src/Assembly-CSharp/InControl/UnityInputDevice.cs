...
public class UnityInputDevice : InputDevice
{
	...
	public UnityInputDevice(UnityInputDeviceProfileBase deviceProfile, int joystickId, string joystickName)
	{
		...
		if (IsKnown)
		{
			...
			for (int i = 0; i < analogCount; i++)
			{
				...
				{
					Debug.LogError((object)string.Concat("Cannot map control \"", inputControlMapping.Handle, "\" as InputControlType.", inputControlMapping.Target, " in profile \"", deviceProfile.Name, "\" because this target is reserved as an alias. The mapping will be ignored."));
				}
				else
				{
			...
			for (int j = 0; j < buttonCount; j++)
			{
				...
				{
					Debug.LogError((object)string.Concat("Cannot map control \"", inputControlMapping2.Handle, "\" as InputControlType.", inputControlMapping2.Target, " in profile \"", deviceProfile.Name, "\" because this target is reserved as an alias. The mapping will be ignored."));
				}
				else
				{
	...
	internal override bool ReadRawButtonState(int index)
	{
		if (index < 20)
		{
			string text = buttonQueries[JoystickId - 1, index];
	...
	internal override float ReadRawAnalogValue(int index)
	{
		if (index < 20)
		{
			string text = analogQueries[JoystickId - 1, index];
